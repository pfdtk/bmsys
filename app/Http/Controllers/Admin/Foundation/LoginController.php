...
{
    /**
     * 登录页面，如果没有登录会显示登录页面, 否则跳转到后台首页
     *
     * @access public
     */
    public function index(LoginProcess $loginProcess)
    {
        $isLogin = $loginProcess->getProcess()->hasLogin();
        if($isLogin) return redirect(route('foundation.index.index'));
        return response(view('admin.login.home'));
    }
    ...
    public function getProc(LoginProcess $loginProcess, Acl $aclObj)
    {
        ...
        $success = ['msg' => '登录成功', 'result' => true, 'jumpUrl' => route('foundation.index.index')];
